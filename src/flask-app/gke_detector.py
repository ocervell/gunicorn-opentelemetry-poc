...
def _get_google_metadata_and_common_attributes():
    ...
    all_metadata = requests.get(_GCP_METADATA_URL,
                                headers=_GCP_METADATA_URL_HEADER).json()
    detach(token)
    common_attributes = {
...
def get_gke_resources():
    ...
    common_attributes.update({
        "k8s.cluster.name":
        all_metadata["instance"]["attributes"]["cluster-name"],
        "k8s.namespace.name":
        pod_namespace,
        "k8s.pod.name":
        os.getenv("POD_NAME", os.getenv("HOSTNAME", "")),
        "host.id":
        all_metadata["instance"]["id"],
        "container.name":
        container_name,
        "gcp.resource_type":
        "gke_container",

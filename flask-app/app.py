...
# Method 2: Export to Cloud Ops directly
...
metrics.get_meter_provider().start_pipeline(meter, exporter, interval=1)
...
# Custom metrics
staging_labels = {"environment": "staging", "app": "flask-app-tutorial"}
hello_requests_counter = meter.create_metric(
    name="hello_requests",
    description="A test counter for hello requests",
    unit="1",
    value_type=int,
    metric_type=Counter,
    label_keys=("environment", "pid", "app",),
)
...
#app.logger.info(f'OpenTelemetry endpoint: {OTEL_AGENT_ENDPOINT}')

@app.route("/")
def hello():
    hello_requests_counter.add(1, staging_labels)
